...
c    group velocity. the rest of abuf contains w(1..2    4.951658       0.3092454        3233.678        6.389455        509.6824      -0.6987377E-09
c    npts) and wp(1..npts)
...
c     if(jcom.lt.1.or.jcom.gt.4) jcom=3
      print *,'enter jcom,lmin,lmax,wmin,wmax,nmin,nmax'
      read(*,*) jcom,lmin,lmax,wmin,wmax,normin,normax
c MB added one line below
...
c		added argument for number of times called (mhr)
	ncall = ncall + 1
      call rotspl(eps1,wt,iout,ifreq,ncall)
   10 continue
   50 continue
      return
      end

      subroutine rotspl(eps1,wt,iout,ifreq,ncall)
...
c        print 200,nord,kchar(jcom),l,cvel,wmhz,tcom,gcom
        write(iout,200) nord,l,wmhz,tcom
      end if
      wtry=b+2.d0*cg*wn
      wt(1)=b
      bm=b
